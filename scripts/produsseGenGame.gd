...
func add_random_connection(out_line_graph: AStar, room_graph: AStar):
	...
	set_tile(start_pos.x, start_pos.y, Tile.clean)
	set_tile(end_pos.x, end_pos.y, Tile.clean)
	
	for pos in path:
		set_tile(pos.x, pos.y, Tile.clean)
...
func add_room(free_regions):
	...
	for y in range(start_y, start_y + size_y):
		...
		set_tile(start_y + size_y - 1, y, Tile.clean)
